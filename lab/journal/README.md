[README](../../README.md) | [Lab](../README.md) | [Plans](../plans/README.md) | [Decisions](../decisions/README.md) | [Journal Template](../templates/journal-template.md)

# Journal

Journal entries are compact lab notes. They should record what was tried, what was learned, and what changed because of it.

## Conventions

- Use `YYYY-MM-DD-slug.md` filenames.
- Write an entry when an experiment, review, or discarded direction teaches the project something useful.
- Prefer facts, findings, and consequences over narrative.
- Link to the plan or decision the note influenced.

## Entries

- [2026-05-15 — Initial Repo Read And Planning](./2026-05-15-initial-repo-read-and-planning.md)
- [2026-05-15 — OpenAI Seed Shape For M2](./2026-05-15-openai-seed-shape.md)
- [2026-05-15 — Claude Seed And Typed Model Contract](./2026-05-15-claude-seed-and-typed-models.md)
- [2026-05-15 — Gemini Seed And Extraction Checkpoint](./2026-05-15-gemini-seed-and-extraction-checkpoint.md)
- [2026-05-15 — Stabilization: Secrets, Backon, And Constructor Shape](./2026-05-15-stabilization-secrets-and-backon.md)
- [2026-05-15 — M3 Multi-Provider Embedding Direction](./2026-05-15-m3-multi-provider-embedding.md)

## See Also

- [Lab Home](../README.md)
- [Plans Index](../plans/README.md)
- [Decision Index](../decisions/README.md)
